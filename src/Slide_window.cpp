...
Slide_window::Slide_window(int x, int y, int xw, int yw, int level, uint64_t inputAddr, uint64_t edgeAddr,
        ...
        : x(x), y(y), xw(xw), yw(yw),
          ...
          input_len(inputLen),
          edge_len(edgeLen),
          output_len(outputLen), num_nodes_in_window(numNodesInWindow), current_node_size(currentNodeSize) {}
...
int Slide_window::getCurrentNodeSize() const {
    ...
    for (auto level_i = 0; level_i < totalLevel - 1; level_i++) {
        ...
        while (col_i < m_graph->get_num_nodes()) {
            ...
            while (row_i < m_graph->get_num_nodes()) {
                ...
                uint64_t input_addr = node_addrs.at(level_i) + row_i * node_size_s.at(level_i) * single_node_size;

                uint64_t edge_addr = m_graph->get_edge_addr(edge_index.at(col_i));

                uint64_t output_addr =
                        node_addrs.at(level_i + 1) + col_i * node_size_s.at(level_i + 1) * single_node_size;
                int input_len = (row_end - row_i) * node_size_s.at(level_i) * single_node_size;

                auto edge_len = (edge_index.at(col_end) -
                                 edge_index.at(col_i)) * 4;

                int output_len = (col_end - col_i) * node_size_s.at(level_i + 1) * single_node_size;
